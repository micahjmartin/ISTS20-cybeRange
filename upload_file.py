...
mal_names = ["l33t_warez.sh", "dank_shellz", "meterpreter", "code.txt", "test.sh", "ls", "whoami", "a.sh", "a", "tmp", "v2", "v6"]
...
def generate_script(type="bash"):
    ...
    retval = retval.format(flag=flag1, flag2=flag2_b64, flag3=flag3)
...
def test(server):
    ...
    response = requests.post('{}/upload'.format(server),
    ...
    if response.status_code != 200:
        print("FAIL: Submission of the script failed. Code:", response.status_code, "Content:", content)
        return
    ...
    if resp.status_code != 200:
        print("FAIL: Getting results failed. Code:", resp.status_code, "Content:", content)
        return
